...
import com.vip.pda.adapter.Util;
import com.vip.pda.adapter.ViewHolder;
import com.vip.pda.adapter.interfaces.OnLoadMoreListener;
...
public abstract class BaseAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public BaseAdapter(Context context, List<T> datas, boolean isOpenLoadMore) {
        mContext = context;
        mData = datas == null ? new ArrayList<T>() : datas;
    ...
    public int getItemCount() {
        if (mData.isEmpty() && (mEmptyView != null || mReloadView != null)) {
            return 1;
        }
        return mData.size() + getFooterViewCount() + getHeaderCount();
    }
    ...
    public int getItemViewType(int position) {
        ...
        return getViewType(position - getHeaderCount(), mData.get(position - getHeaderCount()));
    }
    ...
    public boolean isReloadView(int position) {
        return getItemViewType(position) == TYPE_RELOAD_VIEW;
    }
            ...
            public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
                ...
                if (findLastVisibleItemPosition(layoutManager) + 1 == getItemCount()) {
                    if (mEmptyView != null || mReloadView != null
                            || (mHeaderViews.size() > 0 && showHeaderView && mData.isEmpty())) {
                        return;
                    }
    ...
    private int getFooterViewCount() {
        return isOpenLoadMore && !mData.isEmpty() ? 1 : 0;
    }
